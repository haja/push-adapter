...
public class GcmDatabase extends SQLiteOpenHelper {
  ...
  public void onCreate(SQLiteDatabase db) {
    ...
    // TODO FIXME this should only be needed for google compatibility
    // importLegacyData(db);
  }
  ...
  public synchronized void noteAppRegistered(String packageName, String signature,
  ...
  public synchronized void noteAppUnregistered(String packageName, String signature) {
    SQLiteDatabase db = getWritableDatabase();
    db.delete(TABLE_REGISTRATIONS,
        FIELD_PACKAGE_NAME + " LIKE ? AND " + FIELD_SIGNATURE + " LIKE ?",
  ...
  private App getApp(SQLiteDatabase db, String packageName) {
    Cursor cursor =
        db.query(TABLE_APPS, null, FIELD_PACKAGE_NAME + " LIKE ?", new String[]{packageName}, null,
            null, null, "1");
    if (cursor != null) {
      try {
  ...
  private Registration getRegistration(SQLiteDatabase db, String packageName, String signature) {
    Cursor cursor = db.query(TABLE_REGISTRATIONS, null,
        FIELD_PACKAGE_NAME + " LIKE ? AND " + FIELD_SIGNATURE + " LIKE ?",
        new String[]{packageName, signature}, null, null, null, "1");
    if (cursor != null) {
      try {
        if (cursor.moveToNext()) {
