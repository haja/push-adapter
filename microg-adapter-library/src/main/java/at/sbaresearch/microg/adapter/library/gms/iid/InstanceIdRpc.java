...
public class InstanceIdRpc {
  ...
  public void handleResponseInternal(Intent resultIntent) {
    ...
    if (!ACTION_C2DM_REGISTRATION.equals(resultIntent.getAction()) &&
        !ACTION_INSTANCE_ID.equals(resultIntent.getAction()))
      return;
  ...
  private void sendRegisterMessage(Bundle data, KeyPair keyPair, String requestId) throws
                                                                                   IOException {
    long elapsedRealtime = SystemClock.elapsedRealtime();
    if (nextAttempt != 0 && elapsedRealtime <= nextAttempt) {
      Log.w(TAG,
          "Had to wait for " + interval + ", that's still " + (nextAttempt - elapsedRealtime));
  ...
  private static String sign(KeyPair keyPair, String... payload) {
    ...
    try {
      Signature signature = Signature
          .getInstance(privateKey instanceof RSAPrivateKey ? "SHA256withRSA" : "SHA256withECDSA");
  ...
  private Intent sendRegisterMessageBlockingInternal(Bundle data, KeyPair keyPair) throws
                                                                                   IOException {
